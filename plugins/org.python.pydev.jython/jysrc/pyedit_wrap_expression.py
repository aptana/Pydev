...
Valid when 
==========
The cursor is in a paranthesized (or bracketed, or braced) expression that 
...
class WrapExpression(AssistProposal):
    ...
    def store_data(self, opener_offset, closer_offset, indent):
    ...
    def isValid(self, selection, current_line, editor, offset):
        ...
        if opener_offset is None:
            return False
        closer_offset, has_commas, is_wrapped = get_closer_offset(selection.getDoc().get(), opener_offset, closer)
        if closer_offset < 0:
            if DEBUG_WRAP_EXPRESSION:
        ...
        self.store_data(opener_offset, closer_offset, indent)
        return True
